...
class ThreeBranchModel(nn.Module):
    def __init__(self, pad_len, imu_dim_raw, tof_dim, thm_dim, demo_dim, n_classes, 
        ...
        self.bilstm = nn.LSTM(384, 128, bidirectional=True, batch_first=True)
    ...
    def forward(self, x, demo_features=None):
        ...
        remaining = x[:, :, self.fir_nchan:].transpose(1, 2)  # (batch, tof_dim + thm_dim, seq_len)
        tof = remaining[:, :self.tof_dim, :]  # (batch, tof_dim, seq_len)
        thm = remaining[:, self.tof_dim:self.tof_dim + self.thm_dim, :]  # (batch, thm_dim, seq_len)
